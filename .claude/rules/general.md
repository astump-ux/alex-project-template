...
Before doing anything else in a new conversation, read in this order:
1. **`docs/TOOLSTACK.md`** — canonical stack, accounts, conventions across all my projects
2. **`docs/MCP_INVENTORY.md`** — which MCP tools are available and when to use each
3. **`CLAUDE.md`** — this project's specific architecture notes
4. **`docs/PRD.md`** — what this project is supposed to do
5. **`features/INDEX.md`** — what's been built / planned for this project

If `TOOLSTACK.md` and the project's `CLAUDE.md` disagree, the project's `CLAUDE.md` wins for project-specific decisions. `TOOLSTACK.md` wins for conventions (env var names, account naming, the 3-client Supabase pattern, Sentry config, etc.).

## Language Convention

- **Code, comments, commit messages:** English
- **User-facing content** (UI strings, error messages, marketing copy, business-domain markdown): German
- **Architectural docs** (CLAUDE.md, TOOLSTACK.md, ADRs): English
- Domain-specific German terms in code are acceptable when they map to a real-world concept with no clean English equivalent. Don't force-translate domain language.
...
- **Commit style:** sprint-based + conventional commits hybrid
- **For sprints / multi-commit features:** `Sprint <X>: <high-level description>` followed by bulleted body explaining what's in this commit
- **For single-purpose commits:** `<type>(<scope>): <description>` (e.g. `fix(matching): handle missing invoice date`)
- **Allowed types:** `feat`, `fix`, `refactor`, `docs`, `test`, `chore`, `deploy`, `revert`
- **PROJ-X tagging** is optional — use when a commit relates to a specific tracked feature, omit otherwise
- **Body:** explain *why* — the diff already shows *what*
- **Language:** English (German domain terms OK if they're already in code)
- **Co-author tag for AI-assisted commits:** `Co-Authored-By: Claude <noreply@anthropic.com>` (optional but useful for audit trails)
